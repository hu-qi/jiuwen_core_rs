...
pub fn stable_call_hash(tool_name: &str, tool_args: Option<&Value>) -> String {
    let args = tool_args.cloned().unwrap_or(Value::Object(serde_json::Map::new()));
...
impl Detector for OutputLengthDetector {
    ...
    fn observe(&self, signal: &Signal) -> Option<Anomaly> {
        ...
        if thinking > self.thinking_threshold
            && !fired.contains(&AnomalyKind::ThinkingTooLong)
        {
            ...
            return Some(Anomaly {
                ...
                summary: format!("thinking length {thinking} exceeds {threshold}", threshold = self.thinking_threshold),
                evidence: serde_json::Map::from_iter(vec![
                    ("thinking_len".to_string(), Value::from(thinking)),
                    ("threshold".to_string(), Value::from(self.thinking_threshold)),
                ]),
                peer_member: None,
            });
        }
        ...
        if text > self.text_threshold && !fired.contains(&AnomalyKind::OutputTooLong) {
            ...
            return Some(Anomaly {
                ...
                severity: Severity::Low,
                member_name: signal.member_name.clone(),
                summary: format!("output length {text} exceeds {threshold}", threshold = self.text_threshold),
                evidence: serde_json::Map::from_iter(vec![
    ...
    fn record_and_classify(&self, member: &str, outcome: String) -> Option<Anomaly> {
        ...
        let (severity, kind, evidence) = if identical >= self.global_stop {
            (
                ...
                serde_json::Map::from_iter(vec![("trailing_identical".to_string(), Value::from(identical))]),
            )
            ...
            if alternation >= self.pingpong_warn {
                (
                    ...
                    AnomalyKind::ToolCallLoop,
                    serde_json::Map::from_iter(vec![("trailing_alternation".to_string(), Value::from(alternation))]),
                )
            } else {
                let repeats = state
                    .history
                ...
                if repeats >= self.repeat_warn {
                    (
                        ...
                        serde_json::Map::from_iter(vec![("call_repeats".to_string(), Value::from(repeats))]),
                    )
                } else {
        ...
        Some(Anomaly {
            ...
            summary: format!("{kind:?} detected after {count} calls", count = state.history.len()),
            evidence,
            peer_member: None,
        })
    }
...
mod tests {
    ...
    fn stable_call_hash_order_independent() {
        ...
        // None args 与空对象等价。
        assert_eq!(stable_call_hash("read", None), stable_call_hash("read", Some(&json!({}))));
    }
    ...
    fn output_length_flags_text_and_thinking() {
        ...
        // 非 AfterModelCall 忽略。
        assert!(detector.observe(&Signal::new(SignalKind::BeforeModelCall, "alice")).is_none());
        // reset 后重发。
        detector.reset();
    ...
    fn repeat_detector_alternation_medium() {
        ...
        assert!(anomalies.iter().any(|a| a.severity == Severity::Medium), "{anomalies:?}");
        assert!(
            anomalies.iter().all(|a| a.severity.rank() <= Severity::Medium.rank()),
            "交替上限 Medium"
        );
    }
    ...
    fn plugin_registers_tools_detector() {
        ...
        let detector = ctx.service::<dyn Detector>(&RELIABILITY_TOOLS).expect("tools detector");
