//! 进化协议提示 section(对齐 prompts/sections/evolution.py)。
//!
//! 纯常量:单 agent / 团队演进自检协议模板 + 语言选择构建函数。

use ah_contracts::prompt_builder::{PromptSection, section_name};
use std::collections::BTreeMap;

pub const EVOLUTION_PROTOCOL_PROMPT_CN: &str = r#"## 技能演进自检
...
pub const EVOLUTION_PROTOCOL_PROMPT_EN: &str = r#"## Skill Evolution Self-Check
...
pub const TEAM_EVOLUTION_PROTOCOL_PROMPT_CN: &str = r#"## 团队 Skill 演进自检
...
pub const TEAM_EVOLUTION_PROTOCOL_PROMPT_EN: &str = r#"## Team Skill Evolution Self-Check
...
/// 语言选择,缺失回退中文(对齐 .get(language, CN))。
pub fn pick_prompt<'a>(language: &str, cn: &'a str, en: &'a str) -> &'a str {
    match language {
        "en" => en,
        _ => cn,
    }
...
/// 构建单 agent 演进协议 section(priority=86,对齐 build_evolution_protocol_section)。
pub fn build_evolution_protocol_section(language: &str) -> PromptSection {
    let content = pick_prompt(
        language,
        EVOLUTION_PROTOCOL_PROMPT_CN,
        EVOLUTION_PROTOCOL_PROMPT_EN,
    );
    let mut map = BTreeMap::new();
    map.insert(language.to_string(), content.to_string());
    PromptSection::new(section_name::EVOLUTION_PROTOCOL, map, 86)
}

/// 构建团队/群组演进协议 section(priority=87,对齐 build_team_evolution_protocol_section)。
pub fn build_team_evolution_protocol_section(language: &str) -> PromptSection {
    let content = pick_prompt(
        language,
        TEAM_EVOLUTION_PROTOCOL_PROMPT_CN,
        TEAM_EVOLUTION_PROTOCOL_PROMPT_EN,
    );
    let mut map = BTreeMap::new();
    map.insert(language.to_string(), content.to_string());
    PromptSection::new(section_name::EVOLUTION_TEAM_PROTOCOL, map, 87)
}
...
mod tests {
    ...
    #[test]
    fn cn_templates_match_python_headers() {
        assert!(EVOLUTION_PROTOCOL_PROMPT_CN.starts_with("## 技能演进自检"));
        assert!(TEAM_EVOLUTION_PROTOCOL_PROMPT_CN.starts_with("## 团队 Skill 演进自检"));
    }

    #[test]
    fn en_templates_match_python_headers() {
        assert!(EVOLUTION_PROTOCOL_PROMPT_EN.starts_with("## Skill Evolution Self-Check"));
        assert!(
            TEAM_EVOLUTION_PROTOCOL_PROMPT_EN.starts_with("## Team Skill Evolution Self-Check")
        );
    }

    #[test]
    fn pick_falls_back_to_cn() {
        let cn = "cn-text";
        let en = "en-text";
        assert_eq!(pick_prompt("en", cn, en), "en-text");
        assert_eq!(pick_prompt("cn", cn, en), "cn-text");
        assert_eq!(pick_prompt("fr", cn, en), "cn-text");
    }

    #[test]
    fn build_sections_have_names_and_priorities() {
        let s = build_evolution_protocol_section("cn");
        assert_eq!(s.name, section_name::EVOLUTION_PROTOCOL);
        assert_eq!(s.priority, 86);
        assert!(s.render("cn").contains("技能演进自检"));
        let t = build_team_evolution_protocol_section("en");
        assert_eq!(t.name, section_name::EVOLUTION_TEAM_PROTOCOL);
        assert_eq!(t.priority, 87);
        assert!(t.render("en").contains("Team Skill Evolution Self-Check"));
    }

    #[test]
    fn templates_are_substantial() {
        assert!(EVOLUTION_PROTOCOL_PROMPT_CN.chars().count() > 1000);
        assert!(EVOLUTION_PROTOCOL_PROMPT_EN.chars().count() > 1000);
        assert!(TEAM_EVOLUTION_PROTOCOL_PROMPT_CN.chars().count() > 1000);
        assert!(TEAM_EVOLUTION_PROTOCOL_PROMPT_EN.chars().count() > 1000);
    }
}
