//! 技能沉淀自检提示 section(对齐 prompts/sections/skill_creation.py)。
//!
//! 纯常量:单 agent / 团队技能创建引导 + 本轮团队沉淀提醒模板 + 构建函数。

use ah_contracts::prompt_builder::{PromptSection, section_name};
use std::collections::BTreeMap;

pub const SKILL_CREATION_GUIDANCE_CN: &str = r#"## 技能沉淀自检
...
pub const SKILL_CREATION_GUIDANCE_EN: &str = r#"## Skill Capture Self-Check
...
pub const TEAM_SKILL_CREATION_GUIDANCE_CN: &str = r#"## 团队技能沉淀自检
...
pub const TEAM_SKILL_CREATION_GUIDANCE_EN: &str = r#"## Team Skill Capture Self-Check
...
pub const TEAM_SKILL_CREATION_NUDGE_CN: &str = r#"## 本轮团队技能沉淀检查
...
pub const TEAM_SKILL_CREATION_NUDGE_EN: &str = r#"## Team Skill Capture Check For This Round
...
fn two_lang(cn: &str, en: &str) -> BTreeMap<String, String> {
    let mut m = BTreeMap::new();
    m.insert("cn".to_string(), cn.to_string());
    m.insert("en".to_string(), en.to_string());
    m
}

/// 构建技能创建引导 section(双语言,priority=88)。
pub fn build_skill_creation_guidance_section() -> PromptSection {
    PromptSection::new(
        section_name::SKILL_CREATION_GUIDANCE,
        two_lang(SKILL_CREATION_GUIDANCE_CN, SKILL_CREATION_GUIDANCE_EN),
        88,
    )
}

/// 构建团队技能创建引导 section(双语言,priority=88)。
pub fn build_team_skill_creation_guidance_section() -> PromptSection {
    PromptSection::new(
        section_name::TEAM_SKILL_CREATION_GUIDANCE,
        two_lang(
            TEAM_SKILL_CREATION_GUIDANCE_CN,
            TEAM_SKILL_CREATION_GUIDANCE_EN,
        ),
        88,
    )
}

/// 构建本轮团队技能沉淀提醒 section({skills_dir} 替换为技能目录,priority=89)。
pub fn build_team_skill_creation_nudge_section(skills_dir: &str) -> PromptSection {
    let cn = TEAM_SKILL_CREATION_NUDGE_CN.replace("{skills_dir}", skills_dir);
    let en = TEAM_SKILL_CREATION_NUDGE_EN.replace("{skills_dir}", skills_dir);
    PromptSection::new(
        section_name::TEAM_SKILL_CREATION_NUDGE,
        two_lang(&cn, &en),
        89,
    )
}
...
mod tests {
    ...
    #[test]
    fn cn_templates_have_headers() {
        assert!(SKILL_CREATION_GUIDANCE_CN.starts_with("## 技能沉淀自检"));
        assert!(TEAM_SKILL_CREATION_GUIDANCE_CN.starts_with("## 团队技能沉淀自检"));
        assert!(TEAM_SKILL_CREATION_NUDGE_CN.starts_with("## 本轮团队技能沉淀检查"));
    }

    #[test]
    fn en_templates_have_headers() {
        assert!(SKILL_CREATION_GUIDANCE_EN.starts_with("## Skill Capture Self-Check"));
        assert!(TEAM_SKILL_CREATION_GUIDANCE_EN.starts_with("## Team Skill Capture Self-Check"));
        assert!(
            TEAM_SKILL_CREATION_NUDGE_EN.starts_with("## Team Skill Capture Check For This Round")
        );
    }

    #[test]
    fn guidance_sections_build_with_priorities() {
        let s = build_skill_creation_guidance_section();
        assert_eq!(s.name, section_name::SKILL_CREATION_GUIDANCE);
        assert_eq!(s.priority, 88);
        assert!(s.render("cn").contains("技能沉淀自检"));
        assert!(s.render("en").contains("Skill Capture Self-Check"));
        let t = build_team_skill_creation_guidance_section();
        assert_eq!(t.name, section_name::TEAM_SKILL_CREATION_GUIDANCE);
        assert_eq!(t.priority, 88);
        assert!(t.render("en").contains("Team Skill Capture Self-Check"));
    }

    #[test]
    fn nudge_section_formats_skills_dir() {
        let n = build_team_skill_creation_nudge_section("/tmp/skills");
        assert_eq!(n.name, section_name::TEAM_SKILL_CREATION_NUDGE);
        assert_eq!(n.priority, 89);
        assert!(n.render("cn").contains("/tmp/skills"));
        assert!(n.render("en").contains("/tmp/skills"));
        assert!(!n.render("cn").contains("{skills_dir}"));
    }
}
