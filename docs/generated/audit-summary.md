...
| P3-04 | P3 | providers | Vendor-specific providers | partial |
...
- Status: `partial`
- Implementation: `crates/ah-contracts/src/llm.rs`, `crates/ah-contracts/src/mcp.rs`, `crates/ah-contracts/src/session.rs`, `crates/ah-plugins-openai/src/lib.rs`, `crates/ah-plugins-anthropic/src/lib.rs`, `crates/ah-plugins-session-log/src/lib.rs`, `crates/ah-plugins-agent-loop/src/lib.rs`, `crates/ah-plugins-mcp/src/client.rs`, `crates/ah-plugins-mcp/src/lib.rs`, `crates/ah-plugins-subagent/src/lib.rs`, `crates/ah-plugins-subagents/src/lib.rs`, `crates/ah-app/src/lib.rs`, `profiles/dev.toml`, `profiles/prod.toml`, `docs/config-catalog.md`, `docs/ROADMAP.md`, `docs/capability-map.md`, `crates/ah-app/tests/differential.rs`, `fixtures/subagents.json`, `references/subagents.json`, `differential/run_python.py`
- Verification: `cargo test --offline -p ah-plugins-mcp -p ah-plugins-subagents (19 passed)`, `cargo test --offline -p ah-contracts -p ah-plugins-mcp --lib --tests (245 passed; 0 failed)`, `cargo test --offline -p ah-plugins-openai -p ah-plugins-anthropic --lib (284 passed; 0 failed)`, `adb version (36.0.0)`, `adb devices (10AC5Z0FL1000NL attached and online)`, `adb -s 10AC5Z0FL1000NL shell getprop ro.product.model (V2199A)`, `adb -s 10AC5Z0FL1000NL exec-out screencap -p (92463 bytes)`, `Rust fake MCP stdio server initialize/list_tools/call_tool protocol smoke passed`, `CARGO_TARGET_DIR=/tmp/ah-target-subagents cargo test --offline -p ah-plugins-subagent --lib interruption_during_model_call_is_recorded (1 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-subagents cargo test --offline -p ah-plugins-subagent --lib (6 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-subagents cargo clippy --offline -p ah-plugins-subagent --lib --tests -- -D warnings (passed)`, `cargo fmt --all --check (passed)`, `cargo test --offline -p ah-app --test differential reference_subagents_browser_capabilities (passed)`
- Production: Rust browser proxy and Android ADB paths are implemented and locally protocol-tested; an online Android device is attached and health/screenshot smoke passed; subagent model-call interruption now records AgentInterrupted/AgentCanceled in the session log; full Python lifecycle rails and interactive application-specific mobile flow remain unverified
- Differential: agent-core@aeb88cd8 browser capability resolver: 3 matched, 0 known divergence, 0 mismatch; core/pdf/vision allowlists, duplicate normalization, and unknown capability rejection are covered
...
- Verification: `cargo test -p ah-plugins-retrieval --lib dashscope_embedding_client_posts_native_request_and_parses_response (1 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-retrieval cargo test --offline -p ah-plugins-retrieval --lib dashscope_embedding_client_retries_transient_status (1 passed)`, `cargo test -p ah-plugins-openai --lib config_from_env_ (4 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-rerank cargo test --offline -p ah-plugins-rerank --lib (7 passed; credentials-aware query rerank and concurrency serialization)`, `CARGO_TARGET_DIR=/tmp/ah-target-retrieval cargo test --offline -p ah-plugins-retrieval --lib dashscope_credentials_are_resolved_from_seam (1 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-retrieval-tls cargo test --offline -p ah-plugins-retrieval --lib (13 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-retrieval-tls cargo clippy --offline -p ah-plugins-retrieval --lib --tests -- -D warnings (passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-rerank cargo clippy --offline -p ah-plugins-rerank --lib --tests -- -D warnings (passed)`, `AH_ENV_FILE=$PWD/.env cargo run --offline -q -p ah-app --bin ah-app -- profiles/prod.toml (configured OpenAI-compatible vendor embedding and vector search returned 1 hit)`
- Production: Configured OpenAI-compatible vendor embedding credentials and HTTPS/TLS path passed through the production profile into Redis-backed vector ingest/search; DashScope native embedding/reranking and other real vendor credentials remain pending
- Differential: not applicable
