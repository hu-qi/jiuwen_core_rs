...
> Generated from `audit/ledger.json`; recorded at 2026-09-07; code revision `e3b8f67`; reference revision `aeb88cd8`.
...
- Verification: `current working-tree review`, `cargo fmt --all --check (passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-final-test cargo test --offline --workspace (1491 passed, 252 suites, 9 ignored, 128 filtered)`, `CARGO_TARGET_DIR=/tmp/ah-target-final-clippy cargo clippy --offline --workspace --all-targets -- -D warnings (passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-final-audit cargo run --offline -q -p ah-app --bin audit-ledger -- audit/ledger.json docs/generated/audit-summary.md (generated)`, `git diff --check (passed)`
- Production: not applicable
...
- Status: `done`
- Implementation: `crates/ah-plugins-store/src/lib.rs`, `crates/ah-plugins-store/src/pg_store.rs`, `crates/ah-plugins-store/src/elasticsearch.rs`, `crates/ah-plugins-queue/src/lib.rs`, `crates/ah-plugins-queue/src/pulsar.rs`, `crates/ah-plugins-queue/src/redis_queue.rs`, `crates/ah-plugins-checkpointer/src/lib.rs`, `crates/ah-plugins-checkpointer/src/redis_store.rs`, `crates/ah-plugins-sandbox/src/lib.rs`, `crates/ah-plugins-sandbox/src/remote.rs`, `crates/ah-plugins-retrieval/src/lib.rs`, `crates/ah-plugins-retrieval/src/milvus.rs`, `crates/ah-plugins-telemetry/src/lib.rs`, `crates/ah-app/tests/external_services.rs`
- Verification: `docker exec cc-redis redis-cli ping (PONG)`, `cargo test -p ah-plugins-store --lib redis_kv_roundtrip_with_scan_and_delete (1 passed)`, `cargo test -p ah-plugins-queue --lib redis_queue_publish_consume_backlog_and_channels (1 passed)`, `cargo test -p ah-plugins-telemetry --lib export_otlp_posts_valid_json_to_collector (1 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-checkpointer cargo test --offline -p ah-plugins-checkpointer --lib redis_store_roundtrips_atomic_claim_prefix_and_pipeline (1 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-checkpointer cargo test --offline -p ah-plugins-checkpointer --lib (11 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-checkpointer cargo clippy --offline -p ah-plugins-checkpointer --lib --tests -- -D warnings (passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-pg cargo test --offline -p ah-plugins-store --lib pg_kv_roundtrip_with_scan_and_delete (1 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-pg cargo test --offline -p ah-plugins-store --lib pg_message_store_append_read_channels (1 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-pg cargo test --offline -p ah-plugins-store --lib (8 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-p303 cargo test --offline -p ah-plugins-queue --lib (4 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-p303 cargo test --offline -p ah-plugins-store --lib (8 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-p303 cargo test --offline -p ah-plugins-sandbox --lib (5 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-p303 cargo test --offline -p ah-plugins-retrieval --lib (14 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-p303 cargo clippy --offline -p ah-plugins-queue -p ah-plugins-store -p ah-plugins-sandbox -p ah-plugins-retrieval -p ah-plugins-telemetry --all-targets -- -D warnings (passed)`, `AH_ELASTICSEARCH_URL=http://127.0.0.1:19200 cargo test --offline -p ah-app --test external_services elasticsearch_live_e2e -- --ignored (1 passed against Docker Elasticsearch 8.15.5)`, `AH_GAUSSDB_URL=<openGauss PostgreSQL-wire URL> cargo test --offline -p ah-app --test external_services gaussdb_wire_live_e2e -- --ignored (1 passed against Docker openGauss; temporary image adds OpenBLAS, uses a non-initial e2e_user/e2e database, and PgStore uses portable MERGE upsert)`, `AH_PULSAR_URL=http://127.0.0.1:18083 cargo test --offline -p ah-app --test external_services pulsar_live_e2e -- --ignored (1 passed through Docker Pulsar 3.3.3 broker and Java REST proxy)`, `AH_SANDBOX_REMOTE_URL=http://127.0.0.1:18082 cargo test --offline -p ah-app --test external_services remote_sandbox_live_e2e -- --ignored (1 passed against Docker remote policy service)`, `AH_MILVUS_URL=http://127.0.0.1:19530 AH_MILVUS_COLLECTION=agent_harness_e2e cargo test --offline -p ah-app --test external_services milvus_live_e2e -- --ignored (1 passed against Docker Milvus 2.6.6; collection pre-created via live Milvus SDK)`, `User-approved waiver: DashScope native embedding/rerank live smoke was not rerun after the observed Model.AccessDenied / HTTP 403; no native vendor result is claimed`
- Production: Live Docker Redis/PostgreSQL and OTLP/JSON collector paths passed. Current local Docker closure covers Pulsar 3.3.3 broker plus REST proxy, Elasticsearch 8.15.5, remote sandbox policy service, Milvus 2.6.6 REST entity insert/search/delete, and openGauss PostgreSQL-wire CRUD through the real openGauss server. Anthropic protocol smoke passed against the configured DeepSeek-compatible endpoint. User explicitly waived the remaining DashScope native live smoke; it remains unverified.
- Differential: not verified

### P3-04 — Vendor-specific providers

- Status: `done`
- Implementation: `crates/ah-plugins-retrieval/src/lib.rs`, `crates/ah-plugins-openai/src/lib.rs`, `crates/ah-plugins-credentials/src/lib.rs`, `crates/ah-plugins-rerank/src/lib.rs`, `crates/ah-contracts/src/rerank.rs`, `crates/ah-contracts/src/keys.rs`, `crates/ah-app/src/lib.rs`, `crates/ah-app/tests/external_services.rs`
- Verification: `cargo test -p ah-plugins-retrieval --lib dashscope_embedding_client_posts_native_request_and_parses_response (1 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-retrieval cargo test --offline -p ah-plugins-retrieval --lib dashscope_embedding_client_retries_transient_status (1 passed)`, `cargo test -p ah-plugins-openai --lib config_from_env_ (4 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-rerank cargo test --offline -p ah-plugins-rerank --lib (7 passed; credentials-aware query rerank and concurrency serialization)`, `CARGO_TARGET_DIR=/tmp/ah-target-retrieval cargo test --offline -p ah-plugins-retrieval --lib dashscope_credentials_are_resolved_from_seam (1 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-retrieval-tls cargo test --offline -p ah-plugins-retrieval --lib (13 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-retrieval-tls cargo clippy --offline -p ah-plugins-retrieval --lib --tests -- -D warnings (passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-rerank cargo clippy --offline -p ah-plugins-rerank --lib --tests -- -D warnings (passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-p304 cargo test --offline -p ah-plugins-openai --lib (18 passed)`, `CARGO_TARGET_DIR=/tmp/ah-target-workspace-final cargo test --offline -p ah-plugins-anthropic --lib (7 passed; thinking/text/tool response blocks)`, `AH_ENV_FILE=$PWD/.env CARGO_TARGET_DIR=/tmp/ah-target-p304 cargo run --offline -q -p ah-app --bin ah-app -- profiles/prod.toml (real configured vendor chat/stream/retrieval smoke passed)`, `User-approved waiver: AH_ENV_FILE=$PWD/.env cargo test --offline -p ah-app --test external_services dashscope_live_e2e -- --ignored was not rerun; prior observed embedding/rerank responses were Model.AccessDenied / HTTP 403`, `AH_ENV_FILE=$PWD/.env CARGO_TARGET_DIR=/tmp/ah-target-workspace-final cargo test --offline -p ah-app --test external_services anthropic_live_e2e -- --ignored (1 passed against configured HTTPS protocol endpoint; endpoint is api.deepseek.com/anthropic, not api.anthropic.com)`
- Production: Configured vendor OpenAI-compatible HTTPS credentials passed the real production profile boot and chat/stream/retrieval path. Anthropic protocol smoke passed against the configured DeepSeek-compatible endpoint and now parses thinking/text blocks. User explicitly waived DashScope native embedding/rerank live smoke; it remains unverified and no native vendor result is claimed.
- Differential: not applicable
