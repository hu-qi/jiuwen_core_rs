...
| auto_harness(65 文件) | ah-plugins-auto-harness | assess/plan/implement/verify/commit/publish + 真实 git/CI | git seam 真实落地(本回合:init/add/commit/log/status/diff/branch);CI gate runner 与编排留待后续 |
