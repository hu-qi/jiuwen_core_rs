...
| foundation/prompt | `prompt` seam + ah-plugins-prompt-builder | 模板渲染、结构化 prompt、section 构建器 | 确定性渲染({{var}} + 版本化文件后端已落地);section 构建器已落地(本回合:多语言 PromptSection 注册/替换/排序渲染 + DeepAgent 模式 full/minimal/none 过滤 + 注入清洗(sanitize_path/sanitize_user_content)+ 诊断报告(字符数/估算 token/分 section 统计)) |
