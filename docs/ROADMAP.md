...
| P1-05 | application 完整绑定 | partial(Rust invoke + workflow stream 宿主契约已实现) | 结构化 command、LLM intent、memory/invoke rails、请求级配置和 workflow stream 已接入 Rust `ApplicationRuntime`;dev smoke 与 Rust contract 已通过,真实外部 provider 的 production E2E 仍待完成 |
| P1-06 | controller 完整行为 | partial(Rust 核心与公开调度契约已实现) | LLM intent、状态机、父子任务、多条件过滤、跨 session 并发调度和 snapshot 迁移已接入 Rust `Controller`;dev smoke 与 Rust contract 已通过,独立 production controller E2E 仍待完成 |
| P1-07 | workflow 流式执行 | partial(Rust stream/checkpoint 主链已实现) | `WorkflowStreamSink` 按序发出 `workflow_delta`、`workflow_node`、`workflow_resume`、`workflow_final`;checkpoint 版本头、fsync、损坏记录拒绝和节点恢复已测试;完整外部 production stream E2E 仍待完成 |
...
| P1-10 | 序列化契约版本化 | partial(session/controller/workflow checkpoint 已有版本与迁移读取) | session、controller snapshot、workflow checkpoint 已有 envelope/version 与 legacy 读取;外部协议迁移策略和统一 production 验证仍未完成 |
