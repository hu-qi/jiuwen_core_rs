...
| P2-04 | subagents | partial(`ah-plugins-subagent` + `ah-plugins-subagents` + `ah-plugins-mcp`) | Rust-only browser/mobile path includes Playwright MCP stdio cwd/env/capabilities, standard MCP image forwarding, Android ADB request-scoped device selection, dynamic scroll resolution, key events, health and screenshots, OpenAI/Anthropic image messages and session-log projection; typed browser factory now injects compact probe/card/batch/custom-action helpers for every scoped browser request and prompts enforce evidence/probe ordering; Android serial `10AC5Z0FL1000NL` health (`V2199A`) and screenshot smoke passed; full Python lifecycle rails/probes and application-specific mobile flow remain unverified |
...
| P2-06 | retrieval/memory 生产后端 | partial | Docker Redis KV/queue round trips 已通过真实后端验证; DashScope/OpenAI-compatible embedding、reranker、vector store 和 external memory 的完整 production E2E 仍未全部覆盖 |
...
| 10 | P2-02 rails 长尾 | ✅ 已完成:`ah-plugins-rails` planning/completion/retry/approval 与安全 rail;`ah-plugins-security` tiered policy/overrides、shell/network 分层匹配、HITL `PermissionApprovalProvider` 和 fail-closed pre-execute 已接线;GoalManager、Prompt Attachment、runtime model switching、cancellation callback checkpoints、task lifecycle 和 subagent differential 已验证;P2-02 Rust 核心回归 189 passed、Python differential 35 matched、生产 profile smoke 通过;Python 无等价 callback-control seam 标记 not-comparable |
