...
| P1-05 | application 完整绑定 | partial(Rust invoke + workflow stream 宿主契约已实现) | 结构化 command、LLM intent、memory/invoke rails、请求级配置和 workflow stream 与 Python 公开行为对等;当前 Rust `ApplicationRuntime` 已暴露 `stream(AgentRequest, WorkflowStreamSink)` 并返回结构化 `AgentResult`;Python parity 因缺少等价 runtime 仍未验证 |
| P1-06 | controller 完整行为 | partial(Rust 核心与公开调度契约已实现) | LLM intent、状态机、父子任务、并发调度和 snapshot 迁移通过 differential;当前 Rust `Controller::run_pending` 支持跨 session 并行、session 内排序执行,过滤器支持 user/priority/递归子任务,Python parity 仍未验证 |
| P1-07 | workflow 流式执行 | partial(Rust stream/checkpoint 主链已实现,严格 Python parity 仍待) | `WorkflowStreamSink` 按序发出 `workflow_delta`、`workflow_node`、`workflow_resume`、`workflow_final`;checkpoint 版本头、fsync、损坏记录拒绝和节点恢复已测试。Python ActorManager 的多 producer/source-group、节点级中断恢复和 differential 仍待后续验收 |
...
| P1-10 | 序列化契约版本化 | partial(session/controller/workflow checkpoint 已有版本与迁移读取) | session、controller snapshot、workflow checkpoint、外部协议有 envelope/version 和迁移策略 |
